...
- **An open point written in prose but not in `open-questions.md` does not exist.** This is the rule that makes the line above honest. A question raised in a paragraph of the body, and nowhere else, is invisible to every later phase, to the orchestrator's end-of-run ledger audit, and to `_tracking.md` — it reaches nobody. Before writing the Summary, check that every unresolved question the body raises has a ledger row; raise the row if it doesn't, then name it here.
...
|----------|-------------|-----|
| `_tracking.md` (orchestrator-written; `_recap.md` in folders from before v8.5.0) | §1 Summary | It is already a whole-pipeline summary, built from every artifact's own Summary; a summary of a summary adds nothing. |
...
| `qa-plan-agent`'s tables other than `## Risks` (Coverage Complement, Manual Test Cases, Expected Behaviour Changes, Exploratory Charters, Existing Manual Cases to Re-run, Test Data & Environment, UAT Sign-off) | §2 columns | They are a plan for a human to execute, not findings for a gate to resolve. `## Risks` is the artifact's one Disposition table and uses the standard five columns. |
