...
- **`agents/context-extractor-agent.md`** (+ mirrors) — `tools:` narrowed from `Read, Write, Edit, Bash, Grep, Glob, AskUserQuestion` to `Read, Write, Grep, Glob, AskUserQuestion`. This agent's own contract is read-only scanning plus a single new output file; it never needed `Edit` or `Bash`, and the wider grant existed only as an unused capability the agent's own prose promised not to use.
- **`agents/documentation-agent.md`** (+ mirrors) — `tools:` narrowed to drop `Bash`; every action this agent takes is a file read or a Markdown/doc write, never a command.
