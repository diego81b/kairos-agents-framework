...
- **`.kairos/_qa-regression.md`** (new project-root artifact, written by **`agents/qa-plan-agent.md`**) — the cumulative catalogue of manual cases, fourth file to sit beside `_lessons.md`, `_tech-debt.md` and `decisions/` because it outlives the feature that wrote it. 5b appends the reusable cases it writes with stable `QA-n` IDs and an `Area`, retires those the current change automated or removed, and at the next feature selects the `active` cases whose area that change touched — each row citing the changed file that puts it there. Until now a manual case died with its feature folder, so the retest a QA person actually performs, re-running old manual cases against a new change, had nothing to draw from. Grepping callers finds what the code can show; it cannot find the two-operator scenario someone wrote three features ago.
- **`agents/test-verifier-agent.md`**, **`skills/artifact-bookkeeping/SKILL.md`** — the Acceptance Criteria Mapping table now has three states instead of two: covered by automation (at any level), verified outside the suite (`manual — <Cn> → 5b`, only when a declared `VERIFICATION` row names that `AC-n`), and gap. Only the third counts in `gapIds` and `convergence_signal.ac_gaps`, so only a real gap can drive the Phase 3 loop. A criterion needing two machines used to be reported as a coverage gap, which left the implementer two options: mock the second actor and fake the coverage, or let the loop exit on the thrash check. A unit test proves the code does what it was meant to do; it is not the level at which a use case is verified.
...
- **`agents/pm-agent.md`** — a standalone run asked "how do you want to proceed?" with **Approve** recommended even when the analysis carried a `high` risk, and resolved the whole Risks table as one bundle. It now runs the same per-row Risk Disposition Loop the orchestrator runs — Accept / Mitigate now / Escalate / Defer per row, `low` rows auto-accepted, the same on-demand explain when you ask why instead of choosing — and Approve stops being the recommended option once a row was Escalated. `impact-assessment-agent` already worked this way; `pm-agent` was the outlier.
- **`agents/orchestrator-agent.md`** — at the whole-artifact gate, a `critical`/`high` row that the disposition loop had already resolved stripped **Approve** of its recommendation without giving it to any other option, so a requirements analysis naming one high risk reached the gate with nothing recommended at all. The phases with no pass/fail state (`pm-agent`, `impact-assessment-agent`, `context-extractor-agent`, `bug-triage-agent`, `dependency-audit-agent`) now follow the same carve-out the Phase 3a plan already had: a dispositioned high risk is a bound requirement for the next phase, not a reason to regenerate the artifact.
- **`docs/workflow.md`**, **`docs/setup/claude-code.md`** — the pre-pipeline step told you to run `@context-extractor-agent` and `@impact-assessment-agent`, the exact `@`-mention that dispatches an agent as a subagent and strips `AskUserQuestion` — the defect already fixed for the orchestrator. Both now say to launch them as the session's primary agent, and the Claude Code warning covers every agent that asks something mid-run, not just the orchestrator.
- **`docs/overview.md`**, **`docs/agents.md`**, **`docs/agent-files.md`**, **`docs/setup/vscode.md`**, **`docs/.vitepress/config.js`** — which agents you start yourself was stated differently on every page: the overview counted four standalone agents and the FAQ six, the two `pre` rows sat inside the pipeline table unmarked, two of the six sections never used the word standalone, one setup tree left the context extractor unannotated, and the sidebar was a flat 17-item list. The agent list is now split into **Standalone — you run these** and **Pipeline — the Orchestrator runs these**, the copy page's Quick jump gains a `Started by` column (and the Dependency Audit row it was missing), and both counts are reconciled in one sentence.
...
- **`agents/bug-triage-agent.md`**, **`agents/orchestrator-agent.md`** — a bug report can now be triaged **without leaving the run**. The orchestrator's Bug-Input Check stops being a text advisory and becomes a two-option offer at Step 0e; on accept it dispatches `bug-triage-agent` with the new `mode: orchestrated`, where the agent reproduces, finds the root cause, writes `00c-bug-triage.md`, and returns without a gate of its own — the orchestrator then presents that artifact at its own HITL gate and carries `recommended_entry` into the Effort Check. Until now the advisory named an agent the orchestrator was forbidden to call, so the only way to act on it was to abandon the session, run the agent, and start over.
...
- **`AGENTS.md`**, **`.claude/CLAUDE.md`**, **`commands/setup.md`**, **`commands/view.md`**, **`docs/agents.md`**, **`docs/index.md`**, **`docs/setup/claude-code.md`**, **`docs/setup/kimi-code.md`**, **`docs/setup/opencode.md`** — the agent count said 16 in some places and 17 in others, against the 17 files actually shipped in `agents/` since v8.0.0 took the count from 14 to 17. `/kairos:setup` copies files by that number, so the low count was not only cosmetic. All of them now say 17, and the home page's feature card, which listed the agents by name, had been missing QA Plan Agent since 8.0.0.
- **`docs/overview.md`** — three stale claims: the artifact header was still described as carrying `next agent`, a field 8.0.0 removed; the standalone agents were introduced as "two more" when there are four (Bug Triage and Dependency Audit arrived in 8.0.0, and the Orchestrator can now offer Bug Triage itself); and the `skills/` tree listed 6 of the 11 shipped skills.
- **`docs/faq.md`** — rewritten. It still asked whether you need to learn "all 7 agents" and recommended "Claude Teams", and said nothing about how a run is actually started, the effort bands, where artifacts land, the bug-report entry point, or the deliberate deployment boundary.
- **`docs/setup/templates.md`** — the Bug Fix preset handed you a checklist with no mention of triage, two versions after `bug-triage-agent` shipped. It now says to reproduce and find the root cause first, since that is what decides whether this preset or the full one fits.
...
**BREAKING CHANGE:** the core agent count goes from 14 to 17, and `/kairos:setup` materializes and rewrites `@kairos:` calls for all of them — a project set up under 7.x has 14 agent copies in `.claude/agents/` and will not pick up the three new ones until `/kairos:setup` is re-run. The ledger's `constraints.md` also gains a `Category` column; existing `.kairos/` folders keep working (a legacy 6-column table reads as "no obligation declared") and are migrated in place the first time an agent appends a row.
...
- **`agents/qa-plan-agent.md`** — new optional Phase 5b agent, the manual counterpart to `test-verifier-agent`. It plans the verification an automated suite structurally cannot give: manual and exploratory test cases, regression retest selection, test data and environment needs, and UAT sign-off per acceptance criterion. Every row must trace to an `AC-n`, an uncovered range reported by `test-verifier-agent`, or a caller found at a real `file:line` — a regression risk with no caller in the codebase is dropped rather than written. Runs only after the Phase 3 loop has exited, so the plan describes settled code; its `NEEDS_ATTENTION` never re-invokes an implementer. Selected in the orchestrator's post-implementation question, alongside `release-planner-agent` and `documentation-agent`.
- **`skills/issue-tracker-comment/SKILL.md`** — new "No tracker CLI available" section covering every agent that posts to an issue. `jira` and `glab` are frequently absent on a corporate machine; the post is now guarded by a `command -v` check and degrades to a paste-ready comment body instead of failing a phase gate. `qa-plan-agent` is the first agent for which this step is recommended rather than optional — its reader is a human tester who works in the tracker, not in `.kairos/`.
...
- **`agents/orchestrator-agent.md`** — Phase 5b wired in: agent roster, resume phase order, both selection paths (checkbox question and typed fallback), the announce block, phase execution, artifact tree, and the HITL blocking-status list, which now recognises `NEEDS_ATTENTION`. The new phase runs only after the Phase 3 Loop Actuator and its Guard have resolved, so the QA plan is written against settled code.
- **`agents/impact-assessment-agent.md`** — `recommended_agents` can now recommend `qa-plan-agent`, with the mirror-image rule to `test-verifier-agent`'s: it earns its place where automated verification is thin or absent (the coder path, or behavior a person must judge by eye), and is not recommended for a `simple_fix` already covered by tests.
- **`skills/artifact-bookkeeping/SKILL.md`, `skills/artifact-template/SKILL.md`** — status derivation and required frontmatter fields for the new phase. `qa-plan-agent`'s status is purely count-derived from its `## Risks` table; an acceptance criterion that cannot be verified as written becomes a `high` row there rather than a separate status condition, so one mechanism resolves both.
...
- **`docs/`** — `agents.md`, `agent-files.md`, `overview.md`, `workflow.md`, `agentic-loop.md`, `skills-mcp.md`, every setup guide, and the VitePress sidebar cover the four new agents and three new skills. New "Deliberately Out of Scope" section in `CLAUDE.md` records why deployment execution, post-deploy verification, observability, incident response, load testing, and cost analysis are not part of the framework.

- **Phase artifact frontmatter** — a field now earns its place only if the orchestrator branches on it. Removed `open_dispositions` from every phase (it equalled the row count at write time and was zeroed by the Risk Disposition Loop, so it never carried information the Disposition cells didn't already hold), along with `next_agent`, `checks`, `error_codes_count`, `database_changes_summary`, `selected_option`, `contract_enforcement_summary`, `monitoring_summary`, `rollback_summary`, `docs_touched`, `coverage_basis`, `manual_cases`, and `uat_summary` — each of which existed only to be presence-checked by the contract check that required it. `convergence_signal` keeps only the terms the loop cannot derive from `issues_summary`. Artifacts written before this change stay readable; nothing read the removed fields.
- **`agents/code-reviewer-agent.md`, `agents/test-verifier-agent.md`** — the per-check pass/fail results now live in one place, a `## Checks` table in the body, instead of a frontmatter map the body restated (code-reviewer) or a frontmatter map with no body counterpart at all (test-verifier, which now gains the table).
- **`agents/pm-agent.md`** — the body `## Constraints` table is now a one-line pointer to `ledger/constraints.md`. Every downstream agent already read the ledger and none read the body table, so the two copies only gave a reader the job of working out which one was current.
- **Ledger accounting** — the full re-walk of every `constraints.md` row is now mandatory at `architect-agent` (first accounting pass) and `release-planner-agent` (final accounting) only. Every phase in between updates the rows its own work touched and leaves the rest alone, which is what Lean Mode already did. Rows still reach a terminal state before release, so `release-planner-agent`'s `blocked` rule is unaffected.
- **`agents/pm-agent.md`, `agents/architect-agent.md`** — new Trimmed Mode at `effort: medium`, between Lean Mode (`simple_fix`) and the full process. Use Cases collapse to one line each with no Main Flow, and the architect proposes two design options instead of three. Constraints, acceptance criteria, `## Outcome Criterion`, the data model, API contracts, and error codes stay in full at every effort level — they are contract, and a downstream agent reads each one.
- **`agents/architect-agent.md`** — `## Performance Targets` now runs only when a `PERFORMANCE` or `SCALE` constraint row was declared, the same gating already used for the accessibility and compliance checks. An undeclared target is no longer invented for the implementer to build against and the reviewer to check.
- **`agents/orchestrator-agent.md`** — the HITL gate prints the artifact's `## Summary` block, the dispositioned-row count, and the artifact path, then opens the full file only if asked. Each agent's own "Open in Editor" step is skipped when the orchestrator invoked it, and still runs standalone.

### Fixed

- **`skills/artifact-bookkeeping/SKILL.md`** — §4 required `risk_counts` and `open_dispositions` on the `phase: implementer` execution artifact, which neither implementer has ever written — the orchestrator's Artifact Contract Check would have read every implementer run as malformed and re-run the phase. Both fields are gone from that row; the Phase 3a plan artifact, which does carry `risk_counts`, still requires it.
...
- **`skills/issue-tracker-comment/SKILL.md`** (new) — consolidates the jira/glab/bitbucket comment-posting commands that 12 agents each repeated near-verbatim (~20-25 lines apiece, ~270 duplicate lines total), parameterized by `{output_file}`/`{title}` plus plain/title-prefixed body variants and no-Bash/read-only agent variants. Registered in `docs/skills-mcp.md`, `docs/.vitepress/config.js` sidebar (which also gained the pre-existing missing `coding-discipline` entry, caught in the same pass), and root `CLAUDE.md`.
...
- **`agents/architect-agent.md`, `agents/code-reviewer-agent.md`, `agents/context-extractor-agent.md`, `agents/documentation-agent.md`, `agents/impact-assessment-agent.md`, `agents/implementer-coder-agent.md`, `agents/implementer-tdd-agent.md`, `agents/pm-agent.md`, `agents/release-planner-agent.md`, `agents/retrospective-agent.md`, `agents/security-reviewer-agent.md`, `agents/test-verifier-agent.md`** (+ both mirrors, 12 agents) — each agent's own "Issue Tracker Comment" section shrinks from a full ~20-25 line jira/glab/bitbucket block to a one-line pointer at `skills/issue-tracker-comment/SKILL.md`, stating just its two parameters (`{output_file}`, `{title}`) and any variant (title-prefixed body, no-Bash hand-off, read-only). No behavior change — same commands, same conditions, now defined once instead of twelve times.
...
- **`.opencode/agents/release-planner-agent.md`, `.kimi-code/agents/release-planner-agent.md`** — both mirrors were missing a clarifying sentence about `retrospective-agent` being a separate, standalone, non-orchestrated follow-up not chained to by this agent (pre-existing drift, caught by this change's mirror-sync diff check, unrelated to the Issue Tracker Comment extraction above).
