...
**Run Settings Persistence (mandatory).** Once the human has accepted this announcement — including any correction they replied with — write the final values to `.kairos/$feature_folder/ledger/run.md`, replacing its frontmatter whole:
...
Rewrite it whenever one of these values changes later in the run. Without this file a pipeline resumed in a new session loses every setting but `effort`: the auto-fix budgets fall back to nothing, `quick_fix_mode` is forgotten, and the resume point offers phases the human never selected.
